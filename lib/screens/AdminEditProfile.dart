...
class _AdminEditProfilePageState extends State<AdminEditProfilePage> {
  ...
  final _staffIdCtl = TextEditingController();
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      setState(() {
        ...
        _petNameCtl.text = (data['petName'] ?? '').toString();
        _petType = (data['petType'] ?? _petType).toString();
        _staffIdCtl.text = (data['staffId'] ?? '').toString();
      });
    } catch (e) {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // Also persist pet fields editable by admin
      await _fs.users.doc(u.uid).set({
        'petName': _petNameCtl.text.trim(),
        'petType': _petType,
        'staffId': _staffIdCtl.text.trim(),
        ...
        child: Form(
          ...
          child: ListView(
            ...
            children: [
              ...
              // Staff ID (admin-only optional)
              TextFormField(
                controller: _staffIdCtl,
                decoration:
                    const InputDecoration(labelText: 'Staff ID (optional)'),
                textInputAction: TextInputAction.next,
              ),
              const SizedBox(height: 12),

              // Pet Name + Type (admin editable)
              Row(
                children: [
                  Expanded(
                    flex: 3,
