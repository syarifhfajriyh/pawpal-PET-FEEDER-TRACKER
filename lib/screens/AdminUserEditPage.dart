...
class _AdminUserEditPageState extends State<AdminUserEditPage> {
  ...
  void initState() {
    ...
    _nameCtl =
        TextEditingController(text: (data['displayName'] ?? '').toString());
    _avatarCtl =
        TextEditingController(text: (data['avatarUrl'] ?? '').toString());
    final r = data['role'];
    ...
    try {
      if (_isMe && _role != 1) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('You cannot demote your own admin account.')),
        );
        return;
      }
